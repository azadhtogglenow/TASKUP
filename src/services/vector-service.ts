...
export class VectorService {
  static async storeChunks(
    ...
    try {
      const chunkData = chunks.map((content, index) => {
        ...
        if (!embeddingVector) {
          throw new Error(`Embedding index mismatch: Vector missing for chunk index ${index}`);
        }
        ...
        return {
          ...
          content,
          chunkIndex: index,
          // ✅ FIX: Explicitly cast array to pgvector string format '[x,y,z,...]'
          // This stops Drizzle from unrolling elements into individual parameters
          embedding: `[${embeddingVector.join(",")}]` as any,
          metadata: {
  ...
  static async hybridSearch(
    ...
    try {
      ...
      // Handle edge case where queryText contains empty/whitespace strings
      const formattedTextQuery = queryText.trim().split(/\s+/).filter(Boolean).join(' | ') || '*';
